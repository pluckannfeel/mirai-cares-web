import { useMutation, useQueryClient } from "react-query";
import { addOne } from "../../core/utils/crudUtils";
import { MedicalInstitution } from "../types/MedicalInstitution";
import { axiosInstance } from "../../api/server";

const addInstitution = async (
  institution: MedicalInstitution
): Promise<MedicalInstitution> => {
  const institution_data_form = {
    ...institution,
  };

  const formData = new FormData();
  formData.append("institution_json", JSON.stringify(institution_data_form));

  const { data } = await axiosInstance.post(
    "medical_institutions/add_medical_institution",
    formData
  );
  return data;
};

export function useAddInstitution() {
  const queryClient = useQueryClient();
  
  const { isLoading, mutateAsync } = useMutation(addInstitution, {
    onSuccess: (newInstitution: MedicalInstitution) => {
      queryClient.setQueryData<MedicalInstitution[]>(
        ["medical_institutions"],
        (oldInstitutions) => addOne(oldInstitutions, newInstitution)
      );
    },
  });

  return { isAdding: isLoading, addInstitution: mutateAsync };
}
